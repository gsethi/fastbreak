...
org.systemsbiology.visualization.transplant.prototype.log = function(msg)
{
	...
	//if(false && window.console && console.debug)
	{
		console.log(msg);
	}
}
...
org.systemsbiology.visualization.transplant.prototype.drawChr = function(vis,depth,oriantation, root,stumpchr,stumpstart,stumpend,rowid,end,scale)
{
	...
	if(depth==0)
		...
		this.label(root,stumpchr,"end",onclickstring)
		*/
	//this.label(root,stumpchr+":"+stumpstart,"end")
	//this.label(end,":"+stumpend,"start")
	...
	for ( var sprout in this.chrs[stumpchr].branches )
	{
		...
		if(sprout > stumpstart && sprout < stumpend)
		{
			...
			for(var row = 0; row < this.chrs[stumpchr].branches[sprout].length; row++)
			{
				...
				{
					log("conting not drawn at " + depth);
					contig.drawnNTimes++;
...
org.systemsbiology.visualization.transplant.prototype.draw = function(data, options) {
	...
	for (var row = 0; row < data.getNumberOfRows(); row++) {
		...
		if(includeme==true)
		{
			...
			/*if(this.stumpchr == chr1)
			{
				if (max < pos1)
					max = pos1;
				if(min>pos1)
					min = pos1;
				//TODO: include length of branches on other chromosomes
			}
			if(this.stumpchr == chr2)
			{
				if (max < pos2)
					max = pos2;
				if(min>pos2)
	...
	var groupsof4 = [[]]
	var counterto3 = 0;	
	for(var i=0; i < this.chrClauses.length; i++)
	{
		if(counterto3 == 3)
		{
			groupsof4.push([])
		...
		groupsof4[groupsof4.length-1].push(this.chrClauses[i]);
		counterto3++;
	}
	
	if(this.geneds != undefined)
	{
		for(var i=0;i<groupsof4.length; i++)
		{
			var querystring = "select gene_symbol, chromosome, start, end where "+groupsof4[i].join(" or ")+ " order by start asc";
			this.log("loading: " + querystring +" from " + this.geneds);
			var query = new google.visualization.Query(this.geneds);
			query.setQuery(querystring);
			var vis = this;
			this.genesloading++;
			query.send(function (response) { vis.handleGeneResponse(response); });
		}
	}
...
org.systemsbiology.visualization.transplant.prototype.loadTrackData = function()
{
	...
	var querystring = 'select * where sample_id = \''+this.sample_id+'\' and chr = \''+this.stumpchr+'\' and start > '+this.stumpstart+' and start < '+this.stumpend+ " order by start asc";
	this.log("loading: " + querystring +" from " + this.trackds);
	var query = new google.visualization.Query(this.trackds);
	query.setQuery(querystring);
...
{
	log("recenter response recieved by " + this.SWFid);
	if (response.isError()) {
    	this.log('Error in query: ' + response.getMessage() + ' ' + response.getDetailedMessage());
