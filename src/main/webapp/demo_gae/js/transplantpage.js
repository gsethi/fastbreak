		...
		if(patientsamples[st])
			...
			if(patients[patient].samples.hasOwnProperty(st))
			{
				...
				for (var i in patients[patient].samples[st])
				{
					...
					newhtml+="<td><div id='"+filename+"div' ><center><img src='"+ajloader+"'/><\/center><\/div><br/><center>"+filename.split(".")[0]+"</center><\/td>";
				}
...
function handleSurvivalResponse(response)
{
...
function drawVis(file,data)
{
  	...
  	else
  	{
		...
		vis.draw(data,{chr:document.getElementById('chr').value,depth:document.getElementById('depth').value , start:document.getElementById('start').value, end:document.getElementById('end').value, radius:document.getElementById('radius').value, dataservice:transplantws+'?file='+file, geneds:genedatasource });
		log("vis drawn");
	}
}
...
function loadAll()
{
	...
	{
		var query = new google.visualization.Query(transplantws+'?chr='+document.getElementById('chr').value+'&start='+document.getElementById('start').value+'&end='+document.getElementById('end').value+'&depth='+document.getElementById('depth').value+'&radius='+document.getElementById('radius').value+'&file='+loadthese[i]);
